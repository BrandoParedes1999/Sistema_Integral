...
// --- SEVERIDADES (CLASIFICACIONES) DE COLESTEROL ---
elseif ($tipo == "severidad") {
    $sql = "SELECT DISTINCT clasificacion_colesterol 
            FROM datos_fisicos_alumnos 
            WHERE clasificacion_colesterol IS NOT NULL AND clasificacion_colesterol != ''
            ORDER BY 
                CASE clasificacion_colesterol
...
// --- DATOS DE COLESTEROL CON FILTROS (OPTIMIZADO) ---
elseif ($tipo == "colesterol_datos") {
    $where = [];
    $params = [];
    $types = "";
    ...
    if (!empty($_GET["severidad"])) {
        $where[] = "df.clasificacion_colesterol = ?";
    ...
    $sql = "SELECT 
                df.clasificacion_colesterol,
                COUNT(*) as cantidad
            FROM datos_fisicos_alumnos df
            INNER JOIN alumnos a ON df.matricula_alum = a.matricula_alum
            $whereClause
            GROUP BY df.clasificacion_colesterol
            ORDER BY 
                CASE df.clasificacion_colesterol
                    WHEN 'Sin resultados' THEN 1
                    WHEN 'Bajo' THEN 2
                    WHEN 'Optimo' THEN 3
                    WHEN 'Límite Alto' THEN 4
...
// --- COMPARACIÓN ENTRE FACULTADES (COLESTEROL) ---
elseif ($tipo == "comparar_facultades_colesterol") {
    $where = [];
    $params = [];
    $types = "";
    ...
    $sql = "SELECT 
            ...
            $whereClause
            GROUP BY f.nombre_facultad, df.clasificacion_colesterol
