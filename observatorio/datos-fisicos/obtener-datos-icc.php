...
// --- SEVERIDADES (CLASIFICACIONES) DE ICC ---
elseif ($tipo == "severidad") {
    $sql = "SELECT DISTINCT clasificacion_cintura_cadera 
            FROM datos_fisicos_alumnos 
            WHERE clasificacion_cintura_cadera IS NOT NULL AND clasificacion_cintura_cadera != ''
            ORDER BY 
                CASE clasificacion_cintura_cadera
                    WHEN 'Sin resultados' THEN 1
                    WHEN 'Riesgo Bajo' THEN 2
                    WHEN 'Riesgo Moderado' THEN 3
                    WHEN 'Riesgo Alto' THEN 4
                END";
}

// --- DATOS DE ICC CON FILTROS (OPTIMIZADO) ---
elseif ($tipo == "icc_datos") {
    $where = [];
    $params = [];
    $types = "";
    ...
    if (!empty($_GET["severidad"])) {
        $where[] = "df.clasificacion_cintura_cadera = ?";
    ...
    $sql = "SELECT 
                df.clasificacion_cintura_cadera,
                COUNT(*) as cantidad
            FROM datos_fisicos_alumnos df
            INNER JOIN alumnos a ON df.matricula_alum = a.matricula_alum
            $whereClause
            GROUP BY df.clasificacion_cintura_cadera
            ORDER BY 
                CASE df.clasificacion_cintura_cadera
                    WHEN 'Sin resultados' THEN 1
                    WHEN 'Riesgo Bajo' THEN 2
                    WHEN 'Riesgo Moderado' THEN 3
                    WHEN 'Riesgo Alto' THEN 4
...
// --- COMPARACIÓN ENTRE FACULTADES (ICC) ---
elseif ($tipo == "comparar_facultades_icc") {
    $where = [];
    $params = [];
    $types = "";
    ...
    $sql = "SELECT 
            ...
            $whereClause
            GROUP BY f.nombre_facultad, df.clasificacion_cintura_cadera
