...
// --- SEVERIDADES (CLASIFICACIONES) DE TENSIÓN ARTERIAL ---
elseif ($tipo == "severidad") {
    $sql = "SELECT DISTINCT clasificacion_tension_arterial 
            FROM datos_fisicos_alumnos 
            WHERE clasificacion_tension_arterial IS NOT NULL AND clasificacion_tension_arterial != ''
            ORDER BY 
                CASE clasificacion_tension_arterial
                    WHEN 'Hipotensión' THEN 1
                    WHEN 'Normal' THEN 2
                    WHEN 'Elevada' THEN 3
                    WHEN 'Hipertensión Grado 1' THEN 4
                    WHEN 'Hipertensión Grado 2' THEN 5
                    WHEN 'Crisis Hipertensiva' THEN 6
                END";
}

// --- DATOS DE TENSIÓN ARTERIAL CON FILTROS ---
elseif ($tipo == "tension_datos") {
    $where = [];
    $params = [];
    $types = "";
    ...
    if (!empty($_GET["severidad"])) {
        $where[] = "df.clasificacion_tension_arterial = ?";
    ...
    $sql = "SELECT 
                df.clasificacion_tension_arterial,
                COUNT(*) as cantidad
            FROM datos_fisicos_alumnos df
            INNER JOIN alumnos a ON df.matricula_alum = a.matricula_alum
            $whereClause
            GROUP BY df.clasificacion_tension_arterial
            ORDER BY 
                CASE df.clasificacion_tension_arterial
                    WHEN 'Hipotensión' THEN 1
                    WHEN 'Normal' THEN 2
                    WHEN 'Elevada' THEN 3
                    WHEN 'Hipertensión Grado 1' THEN 4
...
// --- COMPARACIÓN ENTRE FACULTADES (TENSIÓN ARTERIAL) ---
elseif ($tipo == "comparar_facultades_tension") {
    $where = [];
    $params = [];
    $types = "";
    ...
    $sql = "SELECT 
            ...
            $whereClause
            GROUP BY f.nombre_facultad, df.clasificacion_tension_arterial
